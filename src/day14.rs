...
pub fn day_14_challenge_2(config: &Config) -> Result<i128, Error> {
    ...
    // find minima and maxima
    // let x_offset = x1_x2_y1_y2.0 - 2;
    let x_offset = 0; // we need lots of space, so this optimization is less so importat.
                      // initial grid was encoutering bounds errors, so let's make it bigger by 2!
...
fn read_and_parse_input(vec_of_strings: Vec<String>) -> Vec<Vec<coord>> {
    ...
    for vec in vec_of_strings.into_iter() {
        ...
        // let vec_delimited: Vec<&str> = vec.split(||"->").collect();
